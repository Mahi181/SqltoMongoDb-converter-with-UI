...
def sql_to_spec(query):
    ...
    statement = Group(word_match + CONDITIONS + word_match
                      ).setParseAction(lambda t: where_func(t))
    select_fields = Group(SELECT + (word_match | Keyword("*")) +
                          ZeroOrMore(Suppress(",") +
                                    (word_match | Keyword("*")))
                          ).setParseAction(lambda t: select_func(t))

    select_distinct = (SELECT + Suppress(CaselessKeyword('DISTINCT')) + LPAREN
                            + (word_match | Keyword("*"))
                               + ZeroOrMore(Suppress(",")
                               + (word_match | Keyword("*")))
                            + Suppress(RPAREN)).setParseAction(lambda t: select_distinct_func(t))

    select_count = (SELECT + Suppress(CaselessKeyword('COUNT')) + LPAREN
                            + (word_match | Keyword("*"))
                               + ZeroOrMore(Suppress(",")
                               + (word_match | Keyword("*")))
                            + Suppress(RPAREN)).setParseAction(lambda t: select_count_func(t))
    ...
    and_term = (OneOrMore(atom) + ZeroOrMore(AND + atom)
                ).setParseAction(lambda t: combine(t))
    or_term = (and_term + ZeroOrMore(OR + and_term)).setParseAction(lambda t: combine(t))
...
# TODO: Reverse this logic when DEBUG defaults to false.
